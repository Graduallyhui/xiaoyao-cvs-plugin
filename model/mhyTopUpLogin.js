...
export default class mysTopLogin {
    ...
    async GetQrCode(ticket) {
        ...
        let ltokenData = await this.user.getData("exchange", { token: UserData.data?.token?.token, mid: UserData.data?.user_info?.mid }, false)
        ...
        return {
            cookie: `ltoken=${ltokenData.data?.token?.token};ltuid=${UserData.data?.user_info?.aid};cookie_token=${ck.data?.cookie_token}`,
